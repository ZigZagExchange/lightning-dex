...
const zzTokenContractZkSync = new ethers.Contract(
  process.env.ZZ_TOKEN_ZKSYNC_ERA_CONTRACT_ADDRESS,
  ERC20ABI,
  zkSyncProvider
);
...
app.get("/btc_deposit", async (req, res, next) => {
  ...
  if (!valid_outgoing_currencies.includes(outgoing_currency))
    return next(
      "Bad outgoing_currency. Supported currencies: ETH, SOL, ZKSync"
    );
  if (!outgoing_address) return next("Must set outgoing_address");
  if (
    ["ETH", "ZKSync"].includes(outgoing_currency) &&
    !ethers.utils.isAddress(outgoing_address)
  )
    return next("Invalid outgoing_address");

  const deposit_address = await db.query(
    "SELECT * FROM deposit_addresses WHERE deposit_currency=$1 AND outgoing_currency=$2 AND outgoing_address=$3 LIMIT 1",
    ["BTC", outgoing_currency, outgoing_address]
  );

  if (deposit_address.rows.length > 0)
    return res.status(200).json(deposit_address.rows[0]);
  else {
    const addressgen = await exec(
      `${process.env.BITCOIN_CLI_PREFIX} getnewaddress`
    );
    const deposit_address = addressgen.stdout.trim();
    try {
      await db.query(
        "INSERT INTO deposit_addresses (deposit_currency, deposit_address, outgoing_currency, outgoing_address) VALUES ($1,$2,$3,$4)",
        ["BTC", deposit_address, outgoing_currency, outgoing_address]
      );
    } catch (e) {
      return next(e);
    }
    return res
      .status(200)
      .json({ deposit_address, outgoing_currency, outgoing_address });
  }
});

app.get("/history/:address", async (req, res, next) => {
  if (!req.params.address)
    return next("GET /history/:address. Missing address");
  try {
    const bridges = await db.query(
      "SELECT * FROM bridges WHERE LOWER(deposit_address)=$1 OR LOWER(outgoing_address)=$1",
...
function deriveSolanaDepositAddress(depositId) {
  const masterPrivateKey = Buffer.from(
    process.env.SOL_LIQUIDITY_ACCOUNT_PRIV_KEY_BASE58,
    "utf-8"
  );
  const seed = crypto
    .createHash("sha256")
...
app.get("/sol_deposit", async (req, res, next) => {
  ...
  if (outgoingCurrency === "ETH" && !ethers.utils.isAddress(outgoingAddress))
    return next("Invalid outgoing_address");

  const depositId = uuid();
  const depositAddress = deriveSolanaDepositAddress(depositId);
  const expiry =
    Math.floor(new Date().getTime() / 1000) + SOL_DEPOSIT_EXPIRY_PERIOD;
  await db.query(
    "INSERT INTO sol_deposits (id, deposit_address, outgoing_currency, outgoing_address, expiry) VALUES ($1,$2,$3,$4,to_timestamp($5))",
    [depositId, depositAddress, outgoingCurrency, outgoingAddress, expiry]
  );
...
app.get("/prices", async (_, res) => {
  ...
  try {
    ...
    coinCapPrices = await axios
      ...
      .then(({ data }) => ({
        sol_usd: Number(
          data.data.find((asset) => asset.id === "solana").priceUsd
        ),
      }));
  } catch (e) {
...
async function getSolanaBalance() {
  ...
  const solanaBalance = await solanaConnection.getBalance(
    liqudityKeyPair.publicKey
  );
...
async function getEthereumBalance() {
  const balance = await ethersProvider.getBalance(
    process.env.ETHEREUM_LIQUIDITY_ADDRESS
  );
...
async function getZksyncBalance() {
  const ethBalance = await zkSyncProvider.getBalance(
    process.env.ZKSYNC_LIQUIDITY_ADDRESS
  );
  const zzBalance = await zzTokenContractZkSync.balanceOf(
    process.env.ZKSYNC_LIQUIDITY_ADDRESS
  );
  return {
...
async function getZKsyncLiteBalances() {
  ...
  const zkSyncLiteWallet = await zksync.Wallet.fromEthSignerNoKeys(
    viewOnlySigner,
    zkSyncLiteProvider
  );
...
app.get("/get-btc-lp-deposit-address", async (req, res, next) => {
  ...
  const deposit_address = addressgen.stdout.trim();
  if (!ethers.utils.isAddress(outgoing_address)) {
    return next("Invalid outgoing address");
  }
  try {
    await db.query(
      "INSERT INTO lp_deposits (deposit_address, deposit_currency, outgoing_address) VALUES ($1,$2,$3)",
      [deposit_address, "BTC", outgoing_address]
    );
    res.status(200).json({
      deposit_address,
    });
