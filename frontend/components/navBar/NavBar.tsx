...
function NavBar() {
  ...
  return (
    <nav className={styles.container}>
      ...
      <Link href="/pool" className={`${styles.tab_container} ${tab === 4 ? styles.tab_active : ''}`} onClick={() => setTab(4)}>Pools</Link>
      <Link href="/stake" className={`${styles.tab_container} ${tab === 5 ? styles.tab_active : ''}`} onClick={() => setTab(5)}>Stake</Link>
      <Link href="/explorer" className={`${styles.tab_container} ${tab === 6 ? styles.tab_active : ''}`} onClick={() => setTab(6)}>Explorer</Link>
    </nav>
  )
}
