...
const runScript = scriptWrapper(async ({ db }) => {
  ...
  const { rows: bridges } = await db.query(
    `SELECT * FROM bridges WHERE (outgoing_currency='${Assets.ZKSyncLite}' OR outgoing_currency='${Assets.ZZTokenZKSyncLite}') AND paid = false AND outgoing_address is NOT NULL LIMIT 50;`
  );
  ...
  for (let bridge of bridges) {
    const { amountMinusFee, fee, outgoingAmount } = getOutgoingAmountAndFee(
      usdPriceMap,
      bridge.deposit_currency,
      bridge.outgoing_currency,
      bridge.deposit_amount
    );
    if (amountMinusFee <= 0) {
  ...
  for (let i = 0; i < batchTxids.length; i++) {
    ...
    if (updateResult.rowCount === 1) {
      console.log(
        `${bridges[i].deposit_currency} to ${bridges[i].outgoing_currency} executed ${batchTxids[i]}`
      );
    }
  }
});
